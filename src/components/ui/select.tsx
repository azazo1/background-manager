...
>(({ className, children, ...props }, ref) => {
  // Split children to handle text and description separately
  const childArray = React.Children.toArray(children);
  const textChild = childArray.find((child) => typeof child === 'string') || childArray[0];
  const descChild = childArray.find((child, index) => index > 0 && typeof child !== 'string');

  return (
    <SelectPrimitive.Item
      ref={ref}
      className={cn(
        "relative flex w-full cursor-pointer select-none flex-col rounded-sm py-1.5 pl-8 pr-2 text-sm outline-none focus:bg-slate-100 focus:text-slate-900 data-disabled:pointer-events-none data-disabled:opacity-50",
        className
      )}
      {...props}
    >
      ...
      <SelectPrimitive.ItemText>{textChild}</SelectPrimitive.ItemText>
      {descChild}
    </SelectPrimitive.Item>
  );
})
